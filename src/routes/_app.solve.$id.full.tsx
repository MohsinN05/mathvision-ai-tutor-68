...
function FullPage() {
  ...
  const handleDownload = () => {
    const lines = [
      ...
      ``,
      `## Steps`,
      ...record.steps.map((s) => `${s.stepNumber}. ${s.explanation}`),
    ].join("\n");
  ...
  return (
    <>
      ...
      <div className="glass-card mt-4 p-5">
        ...
        <div className="mt-3"><MathRender tex={record.latex} display className="text-xl" /></div>
      </div>

      <div className="glass-card mt-4 p-5">
        <div className="text-[10px] font-semibold tracking-[0.2em] text-muted-foreground">RESULT</div>
        <div className="mt-3 overflow-x-auto"><MathRender tex={record.result} display className="text-2xl" /></div>
      </div>
      ...
      <ol className="mt-6 space-y-3">
        {record.steps.map((s) => (
          <li key={s.stepNumber} className="glass-card p-5">
            ...
            <div className="mt-3 overflow-x-auto"><MathRender tex={s.symbolic} display className="text-base" /></div>
