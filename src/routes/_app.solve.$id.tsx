    ...
    <>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-2 space-y-4">
        ...
        {isFallback ? (
          <div className="glass-card flex items-start gap-3 p-5">
            ...
            <div className="text-sm text-muted-foreground">
              Solver currently unavailable. Configure <code className="text-foreground">WOLFRAM_APP_ID</code> in project secrets to enable symbolic solving.
            </div>
          </div>
        ) : (
          <div className="glass-card p-5">
        ...
        {record.steps.length > 0 && (
          <div className="glass-card p-5">
            <div className="text-[10px] font-semibold tracking-[0.2em] text-muted-foreground">NOTEBOOK PREVIEW</div>
            <ol className="mt-4 space-y-3">
              {record.steps.slice(0, 3).map((s) => (
                <li key={s.stepNumber} className="flex gap-3">
                  <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary/20 text-xs font-semibold text-primary">
                    {s.stepNumber}
                  </span>
